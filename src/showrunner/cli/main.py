...
from showrunner.cli.pipeline import (
    pipeline_assemble,
    pipeline_bootstrap,
...
def run(
    ...
    pipeline_validate(ep_path, skip_validate)

    json_data = _load_episode(ep_path)
    effective_seed = pipeline_determinism(json_data, seed, deterministic)

    episode, manifest, paths, jobs, total_beats, scene_count = pipeline_load_plan(
        ep_path, out, effective_seed
    )

    client = AIServicesClient()
    pipeline_bootstrap(episode, paths, client, skip_bootstrap)

    total_done, _ = pipeline_render(
        episode, manifest, paths, client, max_workers, jobs, total_beats
    )

    final_path, srt_path = pipeline_assemble(jobs, paths, burn_captions)

    pipeline_summary(episode, jobs, total_done, total_beats, scene_count, final_path, srt_path)
